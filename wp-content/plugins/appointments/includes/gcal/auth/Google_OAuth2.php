...
class Google_OAuth2 extends Google_Auth {
  ...
  public function __construct() {
    ...
    if (! empty($App_apiConfig['oauth2_client_id'])) {
      $this->clientId = $App_apiConfig['oauth2_client_id'];
    }

    if (! empty($App_apiConfig['oauth2_client_secret'])) {
      $this->clientSecret = $App_apiConfig['oauth2_client_secret'];
    }
    ...
    if (! empty($App_apiConfig['oauth2_approval_prompt'])) {
      $this->approvalPrompt = $App_apiConfig['oauth2_approval_prompt'];
    }
  }
  ...
  private function refreshTokenRequest($params) {
    ...
    if (200 == $code) {
      ...
      $this->token['created'] = time();
		...
		global $appointments;
      $appointments->log("Error refreshing the OAuth2 token, message: '$body'", $code);
    }
  }
  ...
  public function isAccessTokenExpired() {
    ...
    // If the token is set to expire in the next 20 minutes.
    $expired = ($this->token['created']
        + ($this->token['expires_in'] - 20*60)) < time();
